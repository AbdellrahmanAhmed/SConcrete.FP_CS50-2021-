...
///
/// The SAAnimatedNeumorphicContainer will automatically animate between the old and new values of properties when they change using the provided curve and duration. Its child and descendants are not animated.
///
/// The `depth` value is requied and expected as range from 0 to 1, that means Container's elevation.
class SAAnimatedNeumorphicContainer extends StatelessWidget {
  final double depth;
  final Color color;
  final double width;
  final double height;
  ...
  const SAAnimatedNeumorphicContainer({
    Key key,
    @required this.depth,
